...
#### S3 - Standard
1. S3 standard offers high durability, availability and performance object storage for frequently accessed data 
2. Durability is 99.999999999%                                                                                  
3. Availability is 99.99%                                                                                       
4. The cost of data is high but there is very less charge for accessing the object                              
---
#### S3 - Standard-IA (Infrequent Access)  
1. S3-Standard-IA is for data that is accessed infrequently but requires rapid access when needed                                        
2. The cost of storage is much cheaper than the S3-Standard almost half the price,
 But you are charged heavily for accessing your object 
3. Durability is 99.999999999%                                                                                                           
4. Availability is 99.9%                                                                                                                 
---
#### S3 - Intelligent Tiering       
1. S3- Intelligent tiering is the first cloud storage that automatically reduces your storage cost by automatically
 moving data to most cost effective access tier based on thier access 
2. It works by storing data in tiers                                                                                                                                           
3. If your object is in Infrequent Access tier is accessed, then it is automatically moved back to the
   frequently accessed tier
4. Thier is no retrieval fees when using S3-Intelligent tiering storage class and no additional tiering
   fees when objects are moved back between access tier 
5.Durability is 99.999999999%
6. Availability is 99.9%  
---
#### S3 - OneZoned Infrequent Access
1. S3-one zone-IA is for data that is accessed less frequently but requires rapid access when needed 
2. Data stores data in single availability zone                                                      
3. It is a good choice for storing backup (secondary backup) copies of on premises data              
4. Durability 99.999999999%                                                                          
5. Availability 99.5%                                                                                
---
#### S3 - Glacier

  a) S3 Glacier Instant Retrieval
     1. It is an archive storage class that delivers the lowest cost of storage for long lived data that
     is already accessed                                          
     2. Retrieve data in milliseconds                                                                                                                                 
     3. with S3-glacier-instant-retrieval you can save upto 68% storage cost compared to using the
     standard-Infrequent-Access                                         
     4. S3 Glacier Instant Retrieval is ideal for archive data that need immediate access such as medical images,
     news media assets or user generated content archive 

  b) S3 Glacier Flexible Retrieval
     1. It delivers low cost storage upto 10% lower then S3 - Glacier Instant Retrieval                 
     2. For archive data that is accessed 1 or 2 times in a year and its retrieval                      
     3. For archive data that does not need immediate access but needs the flexibility to retrieve data 

  c) S3 Glaciers - Deep - Archive
     1. It is amazon S3's cheapest storage class             
     2. Data that may be accessed once or twice a year       
     3. Design to retain data for longer period eg: 20 years 
     4. Retrieval time is 12 hours                           
     5. 99.9% availability                                   
