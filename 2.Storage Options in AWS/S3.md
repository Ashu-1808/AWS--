...
##  EC2 to S3 (using IAM role-without AWS-configure):
...
Step 1: Create Resources
 ...
 b. Create an S3 bucket
  ...
  -Select same region as EC2 (recommended).
  -Keep default settings → Click Create.
 c. Give permission (Create an IAM Role with S3 Full Access)
  -Go to IAM Service → Roles → Create Role.
  -Select Trusted Entity: AWS Service.
  -Use case: EC2.
  -Attach policy:
    AmazonS3FullAccess (or custom S3 policy).
    Give Role name (e.g., EC2-S3-Role).
    Click Create Role.
...
-No need to store Access Key or Secret Key in EC2.

Step 2: Connect to EC2
 Login to your instance:
 Switch to root user:
  → sudo su -
 (Optional) Verify AWS CLI is installed:
  → aws --version
 If not installed:
...
Step 3: Verify IAM Role is Attached
 ...
 (Shows attached IAM role name.)

Step 4: Check S3 Buckets
  → aws s3 ls
 (Shows list of available S3 buckets.)
 If access denied:
 Re-check IAM Role attachment.
 Ensure S3 policy is correctly attached.

Step 5: Create a File in EC2
  → touch file1.txt
 Verify file:
  → ls
...
Step 6: Upload File to S3
 ...
 Alternative command (copy instead of move):
  → aws s3 cp file1.txt s3://bucket_name

Step 7: Download File from S3
...
## EC2 to S3 Access Without IAM Role
```
...
Step 2: Create S3 Bucket
 ...
 Keep default settings → Click Create.
Note:
 Amazon S3 is a global service (bucket name must be globally unique).
...
Step 4: Create IAM User (Instead of IAM Role)
 Go to IAM Service.
 ...
 Important:
 Save the Secret Key safely. You cannot retrieve it again.
...
Step 5: Configure AWS CLI on EC2
 On EC2 terminal:
  ...
  Output format (json):(can be kept empty)

 This stores credentials in:
  → ~/.aws/credentials

Step 6: Test EC2 to S3 Access
 List buckets:
  → aws s3 ls
 Upload file to S3:
  → touch test.txt
...
Download file from S3:
 ...
 If commands run successfully ✅
 EC2 can access S3 without IAM Role.

Step 7: Verify from S3 Console
 Open S3 bucket.
 You will see test.txt uploaded from EC2.

Final Flow of Architecture

EC2 Instance → AWS CLI (Access Key & Secret Key) → Amazon S3 Bucket
